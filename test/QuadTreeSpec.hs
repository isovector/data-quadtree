...
import           Data.QuadTree.Internal hiding (getLocation, insert, fill, tile)
...
instance (Monoid a, Arbitrary a) => Arbitrary (QuadTree a) where
  arbitrary = do
    ...
    let x = makeTree r
    n <- elements [0..5]
    inserts <- scale (flip div 4) $ vectorOf n $ (,) <$> subRegion r <*> arbitrary
...
spec = do
  ...
  prop "tightly satisfies / fill" $ \r -> do
    subr <- subRegion r
    let tree = fill @String subr "x" $ makeTree r
  ...
  prop "fill 2x2 adds four values to a QAT" $
    \(RegionAtLeast (Region x y ((+ 1) -> w) ((+ 1) -> h)) :: RegionAtLeast 2 2) -> do
      ...
      let tree = makeTree (Region x y w h)
          res = fill @String (Region (x + x') (y + y') 2 2) "x" tree
  ...
  prop "fill gives you back what you put in" $ \r (v :: Sum Int) -> do
    sub <- subRegion r
    p <- pointInRegion sub
    let tree = fill sub v $ makeTree r
